...
for i in range(0, len(contours)):
    ...
    for i in np.arange(0, int(distance[-1]), EPS):
        ...
        if i % STEP_SIZE == 0:
            ...
            if points and invalid_point(point, image_contours):
                prev_point = None
                curve_points = []
                continue
            points.append(point)
...
cv.imwrite("headshot_triangles.png", image_contours)
# cv.imshow("alan_triangles", image_contours)
cv.waitKey(0)
